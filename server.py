...
# ── DEBUG: Print env var status on startup ──
print(f"[DEBUG] ANTHROPIC_API_KEY exists: {bool(os.getenv('ANTHROPIC_API_KEY'))}")
print(f"[DEBUG] ANTHROPIC_API_KEY length: {len(os.getenv('ANTHROPIC_API_KEY', ''))}")
print(f"[DEBUG] All env vars containing ANTHROPIC: {[k for k in os.environ.keys() if 'ANTHROPIC' in k.upper()]}")
